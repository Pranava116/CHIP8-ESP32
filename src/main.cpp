...
// ===== CHIP-8 INSTANCE & DISPLAY BUFFERS =====
Chip8 chip8;

// Display scaling settings (dynamically calculated for 240x320 or 320x240 screens)
int SCALE = 5;
int X_OFFSET = 0;
int Y_OFFSET = 40;
...
// Track previous frame to only update pixels that change (avoids flicker and speeds up SPI display)
...
void renderDisplay() {
  for (size_t i = 0; i < Chip8::VIDEO_WIDTH * Chip8::VIDEO_HEIGHT; ++i) {
    if (chip8.video[i] != prevVideo[i]) {
      ...
      tft.fillRect(X_OFFSET + col * SCALE, Y_OFFSET + row * SCALE, SCALE, SCALE, color);
...
void setup() {
  ...
  tft.begin();
  tft.setRotation(1); // 320x240 orientation (use setRotation(0) for 240x320 portrait)
  calculateScaling();
  tft.fillScreen(ILI9341_BLACK);

  // Initialize previous frame tracking buffer to 0 (matching screen clear)
